  ...
  plot <- ggplot(nmds_df, aes(x = MDS1, y = MDS2, color = Category)) +
    ...
    theme(
      ...
      legend.title = element_text(
        size = unit(20, "pt"), face = "bold", family = arial_font
      ),
      legend.key = element_rect(fill = "white"),
      legend.text = element_text(size = unit(20, "pt"), family = arial_font),
      axis.title = element_text(
        size = unit(20, "pt"), face = "bold", family = arial_font
      ),
      axis.text = element_text(size = unit(20, "pt"), family = arial_font),
    ...
    annotate(
      "text", x = 0.015, y = 0.1, label = paste("Stress: ", stress), 
...
# Phyla ------------------------------------------------------------------------
phyla_df <- read_csv("data/taxonomic_annotation/phyla_relative_16s_2022.csv")
merged_df <- phyla_df %>%
...
# Family -----------------------------------------------------------------------
family_df <- read_csv("data/taxonomic_annotation/family_relative_16s_2022.csv")
merged_df <- family_df %>%
  inner_join(metadata_df, by = "sample")
data_num <- merged_df %>%
  select(-sample, -tree_richness, -id_plot)
...
# Genus ------------------------------------------------------------------------
genus_df <- read_csv("data/taxonomic_annotation/genus_relative_16s_2022.csv")
merged_df <- genus_df %>%
  inner_join(metadata_df, by = "sample")
data_num <- merged_df %>%
  select(-sample, -tree_richness, -id_plot)
...
ggsave2("results/figures_main/nmds_panel_2022", panel, width = 15, height = 7.5)
